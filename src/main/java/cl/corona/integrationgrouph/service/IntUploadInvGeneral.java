...
@Service
public class IntUploadInvGeneral {
    @Value("${sftpddev.ip}")
    ...
    @Value("${sftpddev.dst_IPHY_g}")
    private String d_sftpdtn_IPHY_g;
    ...
    public void UploadFile() throws IOException {
        ...
        try {
            ...
            for (File file : fList) {
                ...
                switch (sSubCadena) {
                    //FLUJO INVENTARIO GENERAL
                    case "SDIPHYDTI":
                        if (file.isFile()) {
                            String filename = file.getAbsolutePath();
                            //System.out.println(filename + " transfered to --> " + sftpdtn);
                            LOG.info("Uploading IPHY REBUSS " + filename + " ---> " + d_sftpdtn_IPHY_g);
                            d_sftp.put(filename, d_sftpdtn_IPHY_g);
                            file.delete();
